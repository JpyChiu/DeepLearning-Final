...
class DataLoader():
    def __init__(self, train_dataset, val_dataset, img_res=96):
        ...
        with open(self.train_dataset_name, newline='') as csvfile:
            ...
            for a, b in zip(rows, range(0, 1000)):  ##假設是list 第一列的資料就是 檔名 labels
        ...
        with open(self.val_dataset_name, newline='') as csvfile:
            # 讀取 CSV 檔案內容
            rows = csv.reader(csvfile)
            for a, b in zip(rows, range(0, 1000)):
                #採用cv2 讀取像素 附加到list尾端 形成一維list    
    ...
    #把圖片跟年齡都放好  回傳兩者   
    def load_batch(self, batch_size=1):#假設讀1000張圖片
        #n_batches = 1000 / 20 = 50
        self.n_batches = int(len(self.train_data) / batch_size)
        
        for i in range(self.n_batches - 1):#0 ~  50      
            batch = self.train_data[i * batch_size:(i + 1) * batch_size]#[0 * 20 : 1 * 20]  [0:20] = 取第0到20項   #第0個第4個colcol取到
    ...
    def one_hot_encode(self, y, num_classes=7):#改num_classes=7
        return np.squeeze(np.eye(num_classes)[y.reshape(-1)]) #分成batch_size個[0. 0. 0. 0. 0. 0. 1.]這樣的形式 #np.eye(N) NxN對角矩陣
